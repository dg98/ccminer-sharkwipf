 ...
 */
static json_t *
json_rpc_call(CURL *curl, const char *url, const char *userpass, const char *rpc_req, bool longpoll_scan, bool longpoll,
              bool keepalive, int *curl_err) {
...
json_t *json_rpc_call_pool(CURL *curl, struct pool_infos *pool, const char *req, bool longpoll_scan, bool longpoll,
                           int *curl_err) {
...
double target_to_diff(uint32_t *target) {
    ...
    uint64_t m =
            (uint64_t) tgt[29] << 56 | (uint64_t) tgt[28] << 48 | (uint64_t) tgt[27] << 40 | (uint64_t) tgt[26] << 32 |
